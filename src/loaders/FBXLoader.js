 ...
 */
import * as THREE from '../constants';
...
let FBXLoader = ( function () {
	...
	// Parse FBX material and return parameters suitable for a js material
	...
	function parseParameters( FBXTree, properties, textureMap, ID, connections ) {
		...
		connections.get( ID ).children.forEach( function ( child ) {
			...
			switch ( type ) {
				...
				default:
					console.warn( 'FBXLoader: %s map is not supported in js, skipping texture.', type );
					break;
	...
	function getTexture( FBXTree, textureMap, id, connections ) {
		...
		if ( 'LayeredTexture' in FBXTree.Objects && id in FBXTree.Objects.LayeredTexture ) {
			...
			console.warn( 'FBXLoader: layered textures are not supported in js. Discarding all but first layer.' );
	...
	function parseMeshGeometry( FBXTree, relationships, geoNode, deformers ) {
		...
		if ( 'GeometricRotation' in modelNode ) {
			...
			var array = modelNode.GeometricRotation.value.map( degToRad );
			array[ 3 ] = 'ZYX';
			...
			switch ( type ) {
				...
				case 2: // Spot
					...
					if ( lightAttribute.OuterAngle !== undefined ) {
						...
						// while js uses a penumbra between (0, 1) to attenuate the inner angle
	...
	function setModelTransforms( FBXTree, model, modelNode ) {
		...
		if ( 'Lcl_Rotation' in modelNode ) {
			...
			var rotation = modelNode.Lcl_Rotation.value.map( degToRad );
			rotation.push( 'ZYX' );
		...
		if ( 'PreRotation' in modelNode ) {
			...
			var array = modelNode.PreRotation.value.map( degToRad );
			array[ 3 ] = 'ZYX';
	...
	function generateRotationTrack( modelName, curves, initialValue, preRotations ) {
		...
		if ( curves.x !== undefined ) {
			...
			curves.x.values = curves.x.values.map( degToRad );

		}
		if ( curves.y !== undefined ) {

			interpolateRotations( curves.y );
			curves.y.values = curves.y.values.map( degToRad );

		}
		if ( curves.z !== undefined ) {

			interpolateRotations( curves.z );
			curves.z.values = curves.z.values.map( degToRad );
		...
		if ( preRotations !== undefined ) {
			...
			preRotations = preRotations.map( degToRad );
